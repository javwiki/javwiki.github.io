---
name: work-intro
description: 获取 AV 作品的日语官方简介并写入 docs/zh/作品/ 页面（保留日文原文 + 添加中文翻译），并同步日文、英文版。当用户要求为某番号补充简介、翻译作品介绍，或提到从 missAV/FANZA 获取作品介绍时使用。
---

# 获取作品日文简介并写入页面

目标：为 `docs/zh/作品/<番号>.md` 补充「日文原文 + 中文翻译」的官方简介，并同步到 `docs/ja/作品/<番号>.md` 与 `docs/en/作品/<番号>.md`。

三棵树必须结构一致（front matter、标题层级、链接目标、表格行列数、代码块数量），由 `scripts/check_i18n.py` 校验；因此本页新增小节时三个版本都要加，只改文字：

- 中文源：`### 日文原文` / `### 中文翻译`，原文原样保留，中译为助手翻译。
- 日文版：`### 日文原文`（小节名与原文不动）/ `### 中国語訳`（正文仍为中文）。
- 英文版：`### Original Japanese (missAV / FANZA)` / `### Chinese translation`。

其余章节标题按各语言既有写法翻译，勿改动链接目标与文件名。

## 页面格式约定

在中文源的 `## 简介` 或正文末尾、`## 参考资料` 之前插入：
